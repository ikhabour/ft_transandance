  ...
  if (p1Score === 5 || p2Score === 5) {
    dispatchEvent(
      new CustomEvent("game-result", {
        detail: { winner: p1Score > p2Score ? "Player 1" : "Player 2" },
      })
    );
    gamePaused = true;
    p1Score = 0;
...
function updateBall(ball, paddles, Terrain, Scene) {
  ...
  if (ball.position.x <= -7.5 || ball.position.x >= 7.5) {
    if (ball.position.x < 0) updateScore(Scene, 2, Terrain);
    else updateScore(Scene, 1, Terrain);
    // Reset ball position
...
renderer.domElement.addEventListener("keydown", (event) => {
  if (event.code === "Numpad0") singlePlayerMode();
  if (event.code === "ShiftRight") useComposer = !useComposer;
  if (event.code === "Enter") {
    gamestart = !gamestart;
...
function Playermovements(Terrain, event) {
  ...
  if (!singlePlayer) {
    if (keyState["Numpad8"] && paddle2.position.z - 1.25 > wall.position.z) {
      paddle2.position.z -= speed;
    }
    if (keyState["Numpad2"] && paddle2.position.z + 1.25 < wall1.position.z) {
      paddle2.position.z += speed;
    }
  } else {
    if (
...
const animate = async () => {
  ...
  if (gamestart) {
    ...
    updateBall(
      Terrain.ballmesh,
      [Terrain.paddlemesh1, Terrain.paddlemesh],
      Terrain,
      scene
    );
  }
